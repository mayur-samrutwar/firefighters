-- Increase agent movement speeds so cross-globe trips take ~2 min for scouts.
-- Satellite: 2→12, Scout: 5→32, Water drone: 3→20, Heavy tanker: 1.5→10, Supply drone: 3→20.
...
BEGIN
  FOR a IN
    ...
    WHERE battery_pct > 0
  LOOP
    -- Ground agents: move toward target (shortest path for longitude)
    IF a.type IN ('scout', 'water_drone', 'heavy_tanker', 'supply_drone')
       AND a.target_lat IS NOT NULL AND a.target_lng IS NOT NULL THEN
      spd := CASE a.type
        WHEN 'scout' THEN 32
        WHEN 'water_drone' THEN 20
        WHEN 'heavy_tanker' THEN 10
        WHEN 'supply_drone' THEN 20
        ELSE 0
      END;
      -- Shortest-path longitude difference in [-180, 180]
      delta_lng := a.target_lng - a.lng;
      IF delta_lng > 180 THEN delta_lng := delta_lng - 360; ELSIF delta_lng < -180 THEN delta_lng := delta_lng + 360; END IF;
      dist_deg := sqrt(
      ...
      ELSIF dist_deg > 0 THEN
        ...
        new_lng := a.lng + delta_lng * (step / dist_deg);
