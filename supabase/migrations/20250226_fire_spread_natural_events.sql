-- Fire spread to nearby location when certain natural events are active
-- (lightning_storm, strong_winds, drought_zone). At most one new spread fire per tick; cap 25 fires.
...
DECLARE
  ...
  wind_dir text;
  drain_count int;
  -- Fire spread: spawn new fire near an existing one during natural events
  has_spread_event boolean;
  spread_lat numeric;
  spread_lng numeric;
BEGIN
  ...
  -- Fire spread: during lightning_storm, strong_winds, or drought_zone, with probability spawn one new fire near an existing one
  SELECT count(*) INTO fire_count FROM public.fires;
  has_spread_event := EXISTS (
    SELECT 1 FROM public.world_events
    WHERE start_tick <= new_tick AND new_tick < start_tick + duration_ticks
      AND type IN ('lightning_storm', 'strong_winds', 'drought_zone')
  );
  IF fire_count < 25 AND has_spread_event AND fire_count > 0 AND random() < 0.12 THEN
    SELECT f.lat, f.lng INTO spread_lat, spread_lng
    FROM public.fires f
    ORDER BY random()
    LIMIT 1;
    spread_lat := spread_lat + (random() * 2 - 1) * 1.5;
    spread_lng := spread_lng + (random() * 2 - 1) * 1.5;
    spread_lat := greatest(-85, least(85, round(spread_lat::numeric, 6)));
    spread_lng := round(spread_lng::numeric, 6);
    IF spread_lng > 180 THEN spread_lng := spread_lng - 360; ELSIF spread_lng < -180 THEN spread_lng := spread_lng + 360; END IF;
    INSERT INTO public.fires (lat, lng, intensity, created_tick, updated_tick)
    VALUES (spread_lat, spread_lng, floor(random() * 2 + 1)::int, new_tick, new_tick);
  END IF;
