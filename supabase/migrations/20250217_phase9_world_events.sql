-- Phase 9: equipment_malfunction (extra battery drain), strong_winds (fire drift), keep run_ticks unchanged.
...
BEGIN
  ...
  LOOP
    ...
    new_lat := r.lat;
    new_lng := r.lng;
    -- Strong winds: drift fire position in wind direction (~0.25 deg per tick)
    FOR ev IN
      SELECT type, params FROM public.world_events
      WHERE start_tick <= new_tick AND new_tick < start_tick + duration_ticks
