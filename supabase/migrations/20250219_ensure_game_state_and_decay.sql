-- 1) Ensure game_state has row id=1 so game_tick() doesn't RETURN early (SELECT INTO with no row leaves cur_tick NULL).
-- 2) Bump decay so one tick with many fires visibly reduces Earth % (0.002 was ~0.3/tick at 150 intensity, round 100 -> 100).

-- Ensure row exists: insert if missing, then fix NULL earth_life_pct
INSERT INTO public.game_state (id, tick, earth_life_pct, updated_at)
SELECT 1, 0, 100, now()
WHERE NOT EXISTS (SELECT 1 FROM public.game_state WHERE id = 1);

UPDATE public.game_state
SET earth_life_pct = coalesce(earth_life_pct, 100), updated_at = now()
WHERE id = 1 AND earth_life_pct IS NULL;

-- Replace game_tick with slightly stronger decay so UI shows change within a few ticks (was 0.002 -> ~0.3/tick at 150 intensity)
